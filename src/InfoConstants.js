...
import DASHBOARD_BASICS from "./assets/info/DASHBOARD_BASICS.gif"
...
const INFO_CONSTANTS = {
    ...
    WELCOME_GIF: WELCOME,
    DASHBOARD_BASICS_TEXT: 'As described in the "Welcome" section, the dashboard consists of four main components: the Feed, Watchlist, Trollbox, and Dosbox. Each components size is adjustable via the resizing handle and their position is adjustable by simply dragging and dropping. There are certain limitations to the formatting, but for the most part you can orient each component in any way you want. The layout is saved into local storage so however you set it will persist over reloads. To reset a layout to the default settings, delete the value associated with key rgl-7 in your local storage.',
    DASHBOARD_BASICS_GIF: DASHBOARD_BASICS,
