...
const INFO_CONSTANTS = {
    WELCOME_TEXT: 'Welcome to the Wallet Watchoor™ handbook. Wallet Watchoor is a dashboard for the Ethereum mainnet (and potentially other EVM based chains in the future), where you can view transactions from addresses you have followed. All that is required is a web3 wallet provider such as Metamask installed on your browser, which can be downloaded through the various links contained on this site. Your feed periodically updates every 15 seconds with new transactions from your watched addresses if any were detected. While you are watching™, you can chat with fellow watchoors in the Trollbox — a horrible knockoff of the bitmex™ trollbox. There is also an optional Dosbox window you can open where you can play classic Dos games if you get bored. That sums up the general functionality of this site. For more specific info and guidance, click on the more specific topics listed on the left.',
    WELCOME_MP4: WELCOME,
    DASHBOARD_BASICS_TEXT: 'As described in the "Welcome" section, the dashboard consists of five main components: the Feed, Watchlist, Trollbox, and Dosbox. By default, only the first 3 components are displayed as they are the most essential. If you wish to open the dosbox component, you can do so via the button at the top of the feed component. Each components size is adjustable via the resizing handle and their position is adjustable by simply dragging and dropping. To ensure a component does not move or resize from its current orientation, you can click the red lock button at the top of the component and it will lock itself in place. There are certain limitations to the formatting, but for the most part you can orient each component in any way you want. The layout is saved into local storage so however you set it will persist over reloads. If for whatever reason you wish to reset your layout to the default settings, delete the value associated with key rgl-7 in your local storage or simply move everything back manually.',
    DASHBOARD_BASICS_MP4: DASHBOARD_BASICS,
    ADDING_ADDRESS_TEXT: 'Adding an address to your watchlist ensures in the future all transactions sent to or from said address are populated into your feed. To do so, type a valid ENS or ethereum address as well as an alias you wish to give to the address in the two fields at the top of the watchlist. Once added the address will appear at the end of your list. By default, a few popular addresses have been added to your watchlist to get you started. Some of these addresses such as OpenSea, FTX Exchange, and the Uniswap Router V3 are not going to be practical to have on your feed, as they are apart of thousands of transactions every day and are only there initially to quickly exhibit how the app functions without having to wait for a less active wallet to make transactions. It is reccomended that you remove these addresses after you have gotten a feel for how the app functions and instead replace them with addresses you have gathered from people you choose to follow. These could range from influencers, to hedge funds, to ERC-20 or ERC-721 token contracts.',
    ADDING_ADDRESS_MP4: ADDING_ADDRESS,
    DELETING_ADDRESS_TEXT: 'Deleting an address from your watchlist ensures transactions to or from the previously saved address are no longer tracked in your feed. To do so, click on the trash can icon associated with the address you wish to delete. This action is irreversable, though you can simply add an address back if doing so was unintended. You can also mute an address if you simply wish to no longer see their transactions in your feed, but do not want to fully remove it from your list. To do this click on the volume icon opposite the delete button associated with the address you wish to mute.',
    ...
    SHARE_MP4: SHARE,
    BREAK_TEXT: 'Whenever you are tired of watching, you can take a break and play some Dos games in the Dosbox. To play a game, click on the button corresponsing to the game you wish to play.',
    BREAK_MP4: BREAK,
}
