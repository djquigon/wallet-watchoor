...
const WatchListAddAddress = ({newAddress, setNewAddress, newAlias, setNewAlias, handleWatchListAdd}) => {
    return (
        <form className={WatchListCSS.addForm} onSubmit={(e) => handleWatchListAdd(e)}>
            <label htmlFor='addAddress'>Add Address</label>
            <input autoFocus id='addAlias' type='text' placeholder='Enter Alias' required
                value={newAlias} onChange={(e) => setNewAlias(e.target.value)}
            />
            <input autoFocus id='addAddress' type='text' placeholder='Enter Address' required
